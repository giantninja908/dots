...
enum { SchemeNorm, SchemeSel, SchemeSelStat }; /* color schemes */
...
static int drawstatusbar(Monitor *m, int bb, char* text);
...
static void logtext(const char *arg);
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
                else if (ev->x > (x = selmon->ww - (int)TEXTW(stext) + lrpad - 2 * sidepad)) {
                        click = ClkStatusText;
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (!(occ & 1 << i || m->tagset[m->seltags] & 1 << i))
		continue;
		w = TEXTW(tags[i]); //text width
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSelStat : SchemeNorm]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
		if (underline_selected && m->tagset[m->seltags] & 1 << i)
                        drw_rect(drw, x + underline_padding, bh - underline_thickness, w - (underline_padding * 2), underline_thickness, 1, 0);
		x += w;
	}
...
void
logtext(const char *arg){
  fprintf(logfile, "LOG %s", arg);
}
...
updatepreview(void)
{
	...
	for (m = mons; m; m = m->next) {
		m->tagwin = XCreateWindow(dpy, root, m->wx + sidepad, m->by + bh + 2 * vertpad, m->mw / 4, m->mh / 4, 0,
